...
app.post('/register',async function(req, res){
    ...
    if(check!==null){
        ...
        await db.collection("User").insertOne({email:req.body.email,password:hash,name:req.body.name});
...
app.post('/login',async function (req, res) {
    ...
    if(user!==null && bcrypt.compareSync(req.body.password,user.password)){
        let token=jwt.sign({email:email},process.env.TOKEN_SECRET,{expiresIn: "6h"})
...
app.get('/cities/:id',async function(req,res){
    ...
    let city=await db.collection("City").findOne({_id: new ObjectId(cityId),user:user});
...
app.delete('/cities/:id',async function(req, res){
    ...
    await db.collection("City").deleteOne({_id:new ObjectId(req.params.id),user:user});
    res.end()
})
